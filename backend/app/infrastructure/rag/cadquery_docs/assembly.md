...
```python
1door.export("door.step")
2door.export("door.xml")
...
\(\lvert \vec{ v } \rvert\) is the modulus of \(\vec{ v }\), ie. the length of
\(\vec{ v }\).
...
\(\vec{ d_1 } \angle \vec{ d_2 }\) is the angle between \(\vec{ d_1 }\) and
\(\vec{ d_2 }\).
...
\(p_\text{ offset }\) is a plane created from the second object, offset in the plane's normal
direction by `param`, and

-

\(\operatorname{dist}( \vec{ a }, b)\) is the distance between point \(\vec{ a }\) and
plane \(b\).
