# NOTE: OCC（pythonocc-core）依存を OCP（CadQuery 同梱）に置き換え
...
from OCP.BRepBndLib import BRepBndLib  # OCP では関数ではなくクラス: BRepBndLib.Add_s(shape, bbox)
...
def write_stl_file(shape, path: str) -> None:
    """OCP の StlAPI_Writer で TopoDS_Shape を STL に書き出す。"""
...
def create_profile_face(profile: Profile, sketch_plane: CoordSystem):
    ...
    for loop in all_loops[1:]:
        # OCP では .Reversed() が TopoDS_Shape を返すので Wire にキャストし直す
        reversed_wire = TopoDS.Wire_s(loop.Reversed())
        topo_face.Add(reversed_wire)
